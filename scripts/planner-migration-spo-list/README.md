...
$migrationTasks = @()
$plan = m365 planner plan list --ownerGroupName $PlanGroupName | ConvertFrom-Json
...
if ($null -ne $plan) {
    ...
    foreach ($task in $plannerTasks) {
        $taskDetails = m365 planner task details get --taskId $task.id --query '{description: description, checklist: checklist.*.{isChecked: isChecked, title: title} }' | ConvertFrom-Json
